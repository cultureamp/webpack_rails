...
function runWebpackDevServer(opts, done) {
  ...
  opts.dev_server_hmr = opts.dev_server_hmr === false ? false : true
...
function makeDevServerConfig(opts) {
  ...
  if (opts.dev_server_hmr) {
    ...
    // allow webpack-hot-middleware to work in legacy browsers
    devServerClientScripts.push(require.resolve('eventsource-polyfill'));

    // webpack-hot-middleware client
    devServerClientScripts.push(
    ...
    // inject additional client scripts into each bundle
  ...
  // plugin to display useful error messages in browser when build fails
  devServerConfig.plugins.push(new ErrorMessagePlugin());
